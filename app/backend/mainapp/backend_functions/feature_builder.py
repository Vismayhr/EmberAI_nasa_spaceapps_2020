...
class WildfireFeatureBuilder:
	...
	def __init__(self, date_obj, polygon_coords, campsites, power_stations, power_lines):
	...
	def build_features(self):
		...
		polygon_ids = np.array([i for i in range(len(self.campsites))])
		polygon_coords = np.array(self.polygon_coords).T
		weather_features = np.array(self.weather_df).T
		self._features = np.vstack([doy, polygon_ids, polygon_coords[0], polygon_coords[1], self.campsites, self.power_stations, self.power_lines, weather_features[0], weather_features[1], weather_features[2], weather_features[3]]).T
